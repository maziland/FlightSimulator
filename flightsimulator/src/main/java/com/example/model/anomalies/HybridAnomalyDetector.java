		...
		for (firstFeatureIndex = 0; firstFeatureIndex < features.length - 1; firstFeatureIndex++) {
			...
			if (maxCorrelation <= low_correlation) {
				...
				for (int j = 1; j < map.get(features[firstFeatureIndex]).length - 1; j++) {
					float feature_avg = StatLib
							.avg(ZScoreAnomalyDetector.getSlice(map.get(features[firstFeatureIndex]), 0, j));
					float feature_var = StatLib
							.var(ZScoreAnomalyDetector.getSlice(map.get(features[firstFeatureIndex]), 0, j));
		...
		for (int i = 0; i < features.length; i++) {
			if (zscore_map.containsKey(features[i])) {
				...
				for (int j = 1; j < map.get(features[i]).length; j++) {
					...
					if (!c.contains(p)) {
						AnomalyReport anom = new AnomalyReport(
								features[i] + "-" + features[correlated_feature_second_index], i);
						anomalies.add(anom);
					}
				}
			}
